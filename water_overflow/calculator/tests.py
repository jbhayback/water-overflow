...
class CalculatorAppTest(TestCase):
    def test_glass(self):
        ...
        self.assertEqual(Glass(2, 1).get_overfill_content(), -1)
        self.assertEqual(Glass(1, 1).get_overfill_content(), 0)
        self.assertEqual(Glass(1, 2).get_overfill_content(), 1)

    def test_water_overflow(self):
        result = WaterOverflow.get_content(1)
        self.assertEqual(result[0][0].content, 250)
        self.assertEqual(result[2][2].content, 62.5)

        result = WaterOverflow.get_content(0.1)
        self.assertEqual(result[0][0].content, 100)
